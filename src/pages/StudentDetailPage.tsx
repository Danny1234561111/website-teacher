...
const StudentDetailPage: React.FC = () => {
  ...
  const decisionStatusOptions = [
    ...
    { value: 'unknown', label: 'Не указано', color: 'default' as const },
    { value: 'denied', label: 'Отказано', color: 'default' as const },
  ];
  ...
  const getMeetingStatusLabel = (status: string | undefined): string => {
  ...
  const getMeetingStatusColor = (status: string | undefined): "success" | "error" | "default" => {
    if (status?.toLowerCase() === 'met') return 'success';
    if (status?.toLowerCase() === 'not_met') return 'error';
    return 'default';
  };

  const getCallStatusLabel = (status: string | undefined): string => {
  ...
  const getCallStatusColor = (status: string | undefined): "success" | "error" | "default" => {
    if (status?.toLowerCase() === 'reached') return 'success';
    if (status?.toLowerCase() === 'not_reached') return 'error';
    return 'default';
  };

  const getDecisionStatusLabel = (status: string | undefined): string => {
  ...
  const getDecisionStatusColor = (status: string | undefined): "success" | "error" | "warning" | "default" => {
  ...
  const getDocumentsStatusLabel = (status: string | undefined): string => {
  ...
  const getDocumentsStatusColor = (status: string | undefined): "success" | "warning" | "info" | "default" => {
    const statusLower = status?.toLowerCase();
  ...
  // ========== ФУНКЦИИ ДЛЯ ЗВОНКОВ С УЧЕТОМ НАСТРОЕК ==========
  const handleCall = async () => {
    ...
    if (targetDevice === 'pc') {
      ...
      try {
        ...
        if (result.success) {
          if (result.target_device === 'mobile') {
            setSnackbar({ 
              open: true, 
              message: `📞 Звонок инициирован на вашем телефоне`, 
              severity: 'success' 
            });
          } else {
  ...
  // ========== ФУНКЦИИ ДЛЯ SMS С УЧЕТОМ НАСТРОЕК ==========
    ...
    if (targetDevice === 'pc') {
      ...
      try {
        const result = await apiService.sendSmsViaWebSocket(
          student.id, 
          student.phone,
          `Здравствуйте, ${student.full_name}!`
        );
        ...
        if (result.success) {
          if (result.target_device === 'mobile') {
            setSnackbar({ 
              open: true, 
              message: `✉️ SMS открыта на вашем телефоне`, 
              severity: 'success' 
            });
          }
        ...
        setSnackbar({ open: true, message: 'Ошибка при отправке SMS', severity: 'error' });
      }
    }
  };
  ...
  const loadData = async () => {
    ...
    try {
      ...
      setFormData({
        ...
        meeting_status: studentData.meeting_status?.toLowerCase() || 'unknown',
        call_status: studentData.call_status?.toLowerCase() || 'unknown',
        decision_status: studentData.decision_status?.toLowerCase() || 'unknown',
        documents_status: studentData.documents_status?.toLowerCase() || 'unknown',
      });
    } catch (err: any) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setFormData(prev => ({
        ...prev,
        meeting_status: updatedStudent.meeting_status?.toLowerCase() || 'unknown',
        call_status: updatedStudent.call_status?.toLowerCase() || 'unknown',
        decision_status: updatedStudent.decision_status?.toLowerCase() || 'unknown',
        documents_status: updatedStudent.documents_status?.toLowerCase() || 'unknown',
      }));
  ...
  const handleCancelEdit = () => {
    if (student) {
      setFormData({
        ...
        meeting_status: student.meeting_status?.toLowerCase() || 'unknown',
        call_status: student.call_status?.toLowerCase() || 'unknown',
        decision_status: student.decision_status?.toLowerCase() || 'unknown',
        documents_status: student.documents_status?.toLowerCase() || 'unknown',
      });
    }
    setIsEditMode(false);
  };
                  ...
                  <React.Fragment key={app.id}>
                    ...
                    >
                      ...
                      <TableCell align="center">
                        {app.priority ? (
                          <SquareChip 
                            label={app.priority.toString()} 
                            size="small" 
                            color={app.priority === 1 ? 'success' : app.priority === 2 ? 'warning' : 'default'}
                          />
                        ) : '—'}
                      </TableCell>
                      <TableCell align="center">
                              ...
                              (() => {
                                ...
                                return (
                                  <Box className={styles.statsGrid}>
                                    ...
                                    <Box className={styles.statCard}>
                                      <Typography className={styles.statLabel}>Приоритет заявления</Typography>
                                      <Typography className={styles.statValue}>
                                        {app.priority || '—'}
                                      </Typography>
                                    </Box>
    ...
    <Container maxWidth="lg" className={styles.innerContainer}>
      ...
      <Paper className={styles.paperHeader}>
        ...
        <Box className={styles.headerActions}>
          ...
          <IconButton onClick={() => navigate('/profile')} title="Профиль">
            <img 
          ...
          {isEditMode ? (
            <Box sx={{ display: 'flex', gap: 1 }}>
              <IconButton onClick={handleSave} disabled={isSaving}>
                {isSaving ? <CircularProgress size={24} /> : <img src={require('../icons/save.png')} alt="Сохранить" style={{ width: 24, height: 24 }} />}
              </IconButton>
              <IconButton onClick={handleCancelEdit}>
                <img src={require('../icons/cancel.png')} alt="Отмена" style={{ width: 24, height: 24 }} />
              </IconButton>
            </Box>
          ) : (
            <IconButton onClick={() => setIsEditMode(true)}>
              <img src={require('../icons/edit.png')} alt="Редактировать" style={{ width: 24, height: 24 }} />
            </IconButton>
          )}
        </Box>
      </Paper>
                ...
                {isEditMode ? (
                  ...
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, flexWrap: 'wrap' }}>
                    ...
                    {isUrlContact && contactValue && (
                      <Button size="small" startIcon={<OpenInNewIcon />} onClick={() => handleOpenUrl(contactValue)}>
                        Открыть ссылку
                      </Button>
                    )}
                    {isTelegramContact && contactValue && (
                      <Button size="small" startIcon={<TelegramIcon />} onClick={() => handleOpenTelegram(contactValue)}>
                        Открыть Telegram
                      </Button>
                    )}
                  </Box>
